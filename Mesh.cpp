...
void Mesh::initialise(Vertex* vertices, unsigned int numVertices, unsigned int* indices, unsigned int numIndices)
{
	...
	for (unsigned int i = 0; i < numVertices; i++)
	{
		...
		model.texCoordinates.push_back(*vertices[i].GetTexCoordinate());
...
void Mesh::initialiseModel(const IndexedModel& model)
{
	...
	glBufferData(GL_ARRAY_BUFFER, model.positions.size() * sizeof(model.texCoordinates[0]), &model.texCoordinates[0], GL_STATIC_DRAW); //move the data to the GPU - type of data, size of data, starting address (pointer) of data, where do we store the data on the GPU
