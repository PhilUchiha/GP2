...
Screen::Screen()
{
	...
	screenWidth = 1280.0f; // sets the pixel width of screen
	screenHeight = 960.0f; // sets the pixel height of screen
}
...
float Screen::getScreenWidth() { return screenWidth; } //getters
float Screen::getScreenHeight() { return screenHeight; }
...
void Screen::initialiseScreen()
{
	...
	if (sdlWindow == nullptr) // checks screen was created
	{
	...
	if (glContext == nullptr) // checks context was created
	{
		returnError("SDL_GL context failed to create");
	}

	GLenum error = glewInit(); //checks glew is initialised
	if (error != GLEW_OK)
	{
